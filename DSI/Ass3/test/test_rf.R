...
myPreprocess <-  c("knnimpute","center","scale")

getTrControl_rf <- function() {
  # shared bootstrap specification i.e. 25 x bootstrap
  y <- getTrainData()[,"Y"]
  n <- 25
  trainControl(method = "oob", allowParallel = TRUE, search = "random")
}
##############################################################################
...
clus <- startMode(TRUE)
tryCatch({
  tr_res[[method]] <- caret::train(getRecipe(), data = getTrainData(), method = method, metric = "RMSE", trControl = getTrControl_rf(), tuneLength = 15)
},
finally = {
  stopMode(clus)
})

tr_res[[method]]$results[ which.min(tr_res[[method]]$results[, "RMSE"]), ]
tr_res[[method]]$times
plot(tr_res[[method]])
plot(tr_res[[method]]$finalModel)
plot(varImp(tr_res[[method]]))
################
# Create model with default paramters
control <- trainControl(method="repeatedcv", number=10, repeats=3)
mtry <- sqrt(19)
tunegrid <- expand.grid(.mtry=mtry)
tr_res[[method]] <- caret::train(getRecipe(), data=getTrainData(), method="rf", metric="RMSE", tuneGrid=tunegrid, trControl=control)
